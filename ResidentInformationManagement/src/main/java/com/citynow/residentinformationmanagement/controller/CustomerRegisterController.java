...
public class CustomerRegisterController {
    ...
    public List<CustomerRegisterResponse> registerCustomers(@RequestBody List<CustomerRegisterRequest> customerRegisterRequests) {
        List<CustomerRegister.Input> inputList = modelConverter.mapAllByIterator(customerRegisterRequests, CustomerRegister.Input.class);
        return modelConverter.mapAllByIterator(customerRegister.execute(inputList), CustomerRegisterResponse.class);
    }

    @PostMapping("/customers/edit")
    public JsonResponse updateCustomer(@RequestBody @Valid CustomerUpdateRequest customerUpdateRequest, BindingResult result) {
        if (result.hasErrors()) {
            return new JsonResponse("FAIL", result.getAllErrors());
        }
        return new JsonResponse("SUCCESS", modelConverter.map(customerUpdateRequest, CustomerUpdateResponse.class));
    }
}
